...
class OrderForm(StatesGroup):
    ...
    waiting_save_address = State()    # Новый шаг: спросить сохранить ли адрес
...
@router.callback_query(F.data.in_(["cart", "cart_check"]))
async def universal_cart_handler(callback: CallbackQuery):
    """Универсальный обработчик кнопки корзины - для всех мест"""
    try:
        user = await cart_service.get_or_create_user(callback.from_user.id)
        cart_data = await cart_service.get_cart(user.id)

        if not cart_data["items"]:
            # Корзина пуста - показываем всплывающее сообщение
            await callback.answer("🛒 В вашей корзине пока ничего нет", show_alert=True)
            return

        # Корзина не пуста - показываем ее содержимое
        items_text = "\n".join([
...
async def clear_cart(callback: CallbackQuery):
    """Очистить корзину с перенаправлением в главное меню"""
    try:
        user = await cart_service.get_or_create_user(callback.from_user.id)
        cart_data = await cart_service.get_cart(user.id)
...
@router.message(OrderForm.waiting_save_address)
async def process_save_address(message: Message, state: FSMContext):
    """Обработка ответа о сохранении адреса"""
    response = message.text.strip().lower()
    data = await state.get_data()
    
    # Список утвердительных ответов
    yes_responses = ["да", "д", "da", "yes", "y"]
    ...
    if response in yes_responses:
        ...
        if user_id and address:
            await user_service.update_user_info(user_id, address=address)
            await message.answer("✅ Адрес сохранен в профиле!")
    
    # Переходим к проверке telegram login (в любом случае)
    await check_telegram_login_clean(message, state)
...
async def handle_unknown_callback(callback: CallbackQuery):
    ...
    if callback.data in ["main_menu", "catalog", "cart", "cart_check", "profile", "help"]:
        return
    ...
    if callback.data in ["cart", "cart_check"]:
        return
