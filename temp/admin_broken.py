...
from services import update_product_stock_and_availabilityfrom services import update_product_stock_and_availabilityВерсия с исправленной логикой уведомлений
"""
import logging
...
# ДОБАВЛЕННЫЙ ОБРАБОТЧИК - ИСПРАВЛЕНИЕ ПРОБЛЕМЫ
@admin_router.message(AdminStates.waiting_product_unit_type)
async def process_product_unit_type_create(message: Message, state: FSMContext):
    """Обработка единиц измерения товара при создании"""
    unit_choice = message.text.strip()
