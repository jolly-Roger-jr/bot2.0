...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        logger.info("Начало подтверждения заказа")        logger.info(f"Данные заказа: {data}")        data = await state.get_data()
        ...
        logger.info("Создание объекта заказа")            order = Order(
        ...
        logger.info(f"Заказ создан, ID: {order.id}")            await session.refresh(order)
