    ...
    async def add_to_cart(self, user_id: int, product_id: int, quantity: int) -> Dict:
        ...
        async with lock:
            async with get_session() as session:
                ...
                if quantity > product.stock_grams:
                    return {"success": False, "error": f"Недостаточно товара. Доступно: {product.stock_grams}г"}
            ...
            async with get_session() as session:
                ...
                if cart_item:
                    ...
                    if product and new_quantity > product.stock_grams:
                        return {
                            "success": False,
                            "error": f"Максимально доступно: {product.stock_grams}г",
        ...
        async with get_session() as session:
            ...
            for cart_item, product in items:
                ...
                cart_items.append({
                    ...
                    "product_name": product.name,
                    "quantity_grams": cart_item.quantity,
                    "total_price": item_price,
