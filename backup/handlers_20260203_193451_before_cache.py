...
async def handle_quantity(callback: CallbackQuery):
    """Обработка изменения предварительного количества с дебаунсингом"""
    try:
        parts = callback.data.split(":")
        if len(parts) < 3:
            await callback.answer("❌ Ошибка данных", show_alert=True)
        ...
        # БЫСТРО: временно без запроса к БД
        current_in_cart = 0
        # TODO: позже добавить кэширование корзины
        ...
        # ДЕБАУНСИНГ: проверяем нужно ли обновлять UI
        ui_key = f"{callback.from_user.id}_{product_id}"
        current_time = time.time()
        needs_ui_update = True

        if ui_key in last_ui_update:
            time_since_last = current_time - last_ui_update[ui_key]
            if time_since_last < 0.3:  # 300 мс дебаунсинг
                needs_ui_update = False
                # Только быстрый ответ без обновления UI
                unit_suffix = "г" if product.get('unit_type', 'grams') == 'grams' else "шт"
                await callback.answer(f"✅ {new_temp}{unit_suffix}")
                return  # Выходим, UI не обновляем

        if needs_ui_update:
            last_ui_update[ui_key] = current_time
