...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    """Подтверждение и создание заказа с обновлением остатков"""
    try:
        data = await state.get_data()
        ...
        async with get_session() as session:
            # ПРОВЕРЯЕМ ДОСТУПНОСТЬ ВСЕХ ТОВАРОВ ПЕРЕД СОЗДАНИЕМ ЗАКАЗА
            ...
            for item_data in data['cart_items']:
                ...
                availability = await cart_service.is_product_available(product_id, consider_carts=True)
                
                if not availability["available"]:
                    await callback.answer(f"❌ Товар '{item_data['product_name']}' больше не доступен: {availability['reason']}", show_alert=True)
                    # Возвращаем в корзину
                    await show_cart(callback)
                    await state.clear()
                    return
                
                # Проверяем что количество в заказе не превышает доступное
                if quantity > availability["available_stock"]:
                    await callback.answer(
                        f"❌ Для товара '{item_data['product_name']}' доступно только {availability['available_stock']}{'г' if availability.get('unit_type', 'grams') == 'grams' else 'шт'}",
                        show_alert=True
                    )
                    await show_cart(callback)
                    await state.clear()
            ...
            # ЕСЛИ ВСЕ ТОВАРЫ ДОСТУПНЫ - СОЗДАЕМ ЗАКАЗ И СПИСЫВАЕМ ОСТАТКИ
            ...
            # Создаем элементы заказа И СПИСЫВАЕМ ОСТАТКИ
            for item_data in data['cart_items']:
                product_id = item_data['product_id']
                quantity = item_data['quantity']
                ...
                if product:
                    ...
                    # Гарантируем что остаток не уйдет в минус
