...
async def show_products(callback: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            # Если не удалось редактировать, удаляем и отправляем заново
            logger.error(f"Ошибка редактирования в show_products: {edit_error}")
            try:
                await callback.message.delete()
            except:
                pass
...
async def show_product(callback: CallbackQuery):
    ...
    try:
        ...
        caption += price_text
        caption += f"📦 В наличии: {product['stock_grams']}{'г' if product.get('unit_type', 'grams') == 'grams' else 'шт'}\n"
        caption += f"🛒 В корзине: {current_in_cart}{'г' if product.get('unit_type', 'grams') == 'grams' else 'шт'}\n\n"
...
async def back_to_products(callback: CallbackQuery):
    ...
    try:
        ...
        # Удаляем текущее сообщение (с фото или текстом)
        try:
            await callback.message.delete()
        except:
        ...
        # Отправляем новый список товаров
        await callback.bot.send_message(
            chat_id=callback.from_user.id,
            text="📦 Товары\n\nВыберите товар:",
...
async def handle_quantity(callback: CallbackQuery):
    """Обработка изменения предварительного количества"""
    try:
        parts = callback.data.split(":")
        action = parts[0]
        product_id = int(parts[1])
        ...
        # Пробуем обновить подпись (если это фото) или текст (если нет)
        try:
            # Если у сообщения есть photo, это фото с подписью
            if callback.message.photo:
                ...
                # Иначе это текстовое сообщение
                await callback.message.edit_text(
                    caption,  # здесь это текст, не подпись
                    reply_markup=keyboard
                )
        except Exception as edit_error:
            # Если не удалось редактировать, показываем новое сообщение
            ...
            # Если у товара есть изображение, отправляем фото
            if product.get('image_url'):
                await callback.bot.send_photo(
                    chat_id=callback.from_user.id,
                    photo=product['image_url'],
...
async def add_to_cart(callback: CallbackQuery):
    ...
    try:
        ...
        if result["success"]:
            ...
            # Пробуем обновить подпись или текст
            try:
                if callback.message.photo:
                    await callback.message.edit_caption(
                        caption=caption,
...
async def show_profile(callback: CallbackQuery):
    ...
    try:
        ...
        profile_text = (
            ...
            "Чтобы изменить данные, оформите новый заказ."
        )
